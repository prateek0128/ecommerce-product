...
export function useGetCustomer() {
  ...
  const memoizedValue = useMemo(
    () => ({
      customers: data?.customers as TechnicianList[],
...
export async function insertCustomer(newCustomer: TechnicianList) {
  // to update local state based on key
  mutate(
    endpoints.key + endpoints.list,
    (currentCustomer: any) => {
      newCustomer.id = currentCustomer.customers.length + 1;
      const addedCustomer: TechnicianList[] = [...currentCustomer.customers, newCustomer];
...
export async function updateCustomer(customerId: number, updatedCustomer: TechnicianList) {
  // to update local state based on key
  mutate(
    endpoints.key + endpoints.list,
    (currentCustomer: any) => {
      const newCustomer: TechnicianList[] = currentCustomer.customers.map((customer: TechnicianList) =>
...
export async function deleteCustomer(customerId: number) {
  ...
  mutate(
    endpoints.key + endpoints.list,
    (currentCustomer: any) => {
      const nonDeletedCustomer = currentCustomer.customers.filter((customer: TechnicianList) => customer.id !== customerId);
